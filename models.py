...
class LorenTzE_core(nn.Module):
    def __init__(self, embedding_dim, time_dim, entities_num, relations_num, timestamps_num, drop_out, c=1.0, alpha=0.5, bias=-1):
        ...
        # self.T_ct = nn.Embedding(self.timestamps_num, self.embedding_dim)
        #time_mat = torch.empty([self.timestamps_num, self.embedding_dim, self.embedding_dim])
        time_mat = torch.empty([self.timestamps_num, self.embedding_dim, self.time_dim])
        nn.init.xavier_normal_(time_mat)
        self.time_mat = nn.Parameter(time_mat)

        #self.cores = nn.Embedding(self.entities_num, self.embedding_dim)
        self.cores = nn.Embedding(self.entities_num, self.time_dim)
        ...
        self.D_x = nn.Embedding(self.relations_num, self.embedding_dim)
        self.D_y = nn.Embedding(self.relations_num, self.embedding_dim)
        self.D_z = nn.Embedding(self.relations_num, self.embedding_dim)
    ...
    def init(self):
        ...
        nn.init.xavier_normal_(self.R_y.weight.data)
        nn.init.xavier_normal_(self.R_z.weight.data)
        nn.init.xavier_normal_(self.D_x.weight.data)
        nn.init.xavier_normal_(self.D_y.weight.data)
        nn.init.xavier_normal_(self.D_z.weight.data)
    ...
    def forward(self, head, rel, timestamp):
        ...
        '''h_x = h_x * torch.sin(h_ct)
        h_y = h_y * torch.sin(h_ct)
        h_z = h_z * torch.sin(h_ct)'''
    ...
    def loss(self, scores, target, timestamp):
        ...
        '''temp_x = self.E_x(target) * torch.sin(ct_measure)
        temp_y = self.E_y(target) * torch.sin(ct_measure)
        temp_z = self.E_z(target) * torch.sin(ct_measure)'''

        temp_x = self.E_x(target)
        temp_y = self.E_y(target)
        temp_z = self.E_z(target)


        score_x = torch.bmm(t_x.unsqueeze(1), temp_x.unsqueeze(2)).squeeze()
        score_y = torch.bmm(t_y.unsqueeze(1), temp_y.unsqueeze(2)).squeeze()
        score_z = torch.bmm(t_z.unsqueeze(1), temp_z.unsqueeze(2)).squeeze()
    ...
    def neg_loss(self, head, rel, timestamp, neg_label):
        ...
        for i in range(neg_label.shape[1]):
            ...
            '''temp_x = self.E_x(neg_label[:, i]) * torch.sin(ct_measure)
            temp_y = self.E_y(neg_label[:, i]) * torch.sin(ct_measure)
            temp_z = self.E_z(neg_label[:, i]) * torch.sin(ct_measure)'''
            temp_x = self.E_x(neg_label[:, i])
            temp_y = self.E_y(neg_label[:, i])
            temp_z = self.E_z(neg_label[:, i])

            score_x = torch.bmm(neg_x.unsqueeze(1), temp_x.unsqueeze(2)).squeeze()
            score_y = torch.bmm(neg_y.unsqueeze(1), temp_y.unsqueeze(2)).squeeze()
            score_z = torch.bmm(neg_z.unsqueeze(1), temp_z.unsqueeze(2)).squeeze()
